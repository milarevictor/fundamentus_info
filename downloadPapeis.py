...
def designCookie(papel, cookie):
    cookies[papel] = cookie
    return True



def write(r, dir_files, file_name):
...
def main():
    for papel in papel_ativos:
        print("Gerando cookies: [{papel}]".format(papel=papel), end='\r', flush=True)
        if getCookies()['cookie']:
            cookie = getCookies()['cookie']
            designCookie(papel, cookie)

    for papel, SSID in zip(cookies.keys(), cookies.values()):
        print("Definindo cookies: [{papel}]".format(papel=papel), end='\r', flush=True)
        cookiesBHist(papel,SSID)
        try:
            r = requests.get("http://fundamentus.com.br/planilhas.php?SID="+str(SSID), allow_redirects=True)
            open(str(papel)+".zip", 'wb').write(r.content)
        except:
            print("Erro: " + papel)
